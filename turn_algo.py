...
# 회전 감지 임계값
ANGLE_UTURN = 150          # U턴 판정 각도
ANGLE_TURN = 30            # 좌/우회전 판정 최소 각도
MIN_DIST_TURN = 30         # turn 사이 최소 거리(m)
MIN_DIST_SIMPLIFY = 12.0   # polyline 간소화 최소 간격(m)
STRAIGHT_INTERVAL = 250    # 직진 안내 최소 간격(m)
...
def build_turn_by_turn(polyline, km_requested: float, total_length_m: float):
    ...
    estimated_time_min = (total_length_m / 1000.0) / (RUNNING_SPEED_KMH / 60)
    ...
    summary = {
        ...
        "turn_count": len(
            [t for t in turns if t["type"] in ["left", "right", "uturn", "straight"]]
        ),
    }
