...
def loop_score(polyline: List[Dict[str, float]]) -> float:
    ...
    straightness = diag / max(length, 1.0)
...
def generate_loop_route(
    ...
    # 중간지점까지의 대략 거리: 너무 멀지도, 너무 가깝지도 않게
    base_radius = target_m * 0.45
    base_radius = max(400.0, min(base_radius, 2000.0))  # 400m ~ 2km 사이

    candidates = []

    # 방향 × radius 스케일을 섞어서 여러 후보 루프 생성
    for b in bearings:
        for scale in [0.8, 1.0, 1.2]:
            radius_m = base_radius * scale

            # 약간의 랜덤 각도 jitter (루프 다양성 확보)
            ...
            try:
                ...
                # 너무 짧거나 너무 긴 루프는 1차 필터 (느슨하게)
                if length_m < 0.4 * target_m or length_m > 2.0 * target_m:
                    continue
    ...
    candidates.sort(
        key=lambda c: (-c["score"], abs(c["length_m"] - target_m))
    )

    best = candidates[0]
    best_poly = best["polyline"]
    best_len = best["length_m"]
