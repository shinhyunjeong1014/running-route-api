...
# ============================================
# 1. Valhalla 기반 알고리즘 (2km 이상에서 사용)
#    (기존 첫 번째 코드 그대로, 마지막 함수명만 변경)
# ============================================
...
# ----------------------------
# Public API (Valhalla 쪽)
# ----------------------------
def generate_running_route_valhalla(
    lat: float,
    lng: float,
    km: float,
    quality_first: bool = True
...
# ============================================
# 2. OSMNX 기반 Local Loop 알고리즘 (2km 미만에서 사용)
#    (기존 두 번째 코드 그대로)
# ============================================
...
# ==========================
def _to_local_xy_osm(polyline: Polyline) -> List[Tuple[float, float]]:
    """위경도를 평면 좌표계로 근사 변환 (OSM용 별도 함수)."""
    if not polyline:
        return []
...
def generate_running_route(
    ...
    """
    2km 기준으로 두 알고리즘을 엄격 분기:

      - km >= 2.0 : Valhalla 기반 pivot-loop 알고리즘
      - km <  2.0 : OSMNX 기반 Local Loop 알고리즘

    기존 코드와의 호환성을 위해 함수 시그니처는 유지한다.
    """
    km = float(km)

    if km >= 2.0:
        # 2km 이상: 첫 번째 코드
        return generate_running_route_valhalla(lat, lng, km, quality_first=quality_first)
    else:
        # 2km 미만: 두 번째 코드
        poly, meta = generate_area_loop(lat, lng, km)
        dist_km = polyline_length_m(poly) / 1000.0

        return {
            "status": "ok",
            "message": meta.get("message", "근거리 러닝 루프 생성 완료"),
            "start": {"lat": lat, "lng": lng},
